...
const state = {
    ...
    scheduleTranspose: false,  // false = 날짜가 열(column)에 오는 방향 (기본값)
...
async function init() {
    ...
    const searchInputMobile = document.getElementById('mobile-univ-search');
    if (searchInputMobile) {
        searchInputMobile.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                searchCandidatesMobile();
            }
        });
    }
...
function searchCandidatesMobile() {
    const val = document.getElementById('mobile-univ-search').value.trim();
    ...
    renderCandidatesMobile(matched, `🔍 "${val}" 검색 결과`);
}
...
function renderCandidatesMobile(grouped, titleText) {
    const listEl = document.getElementById('mobile-candidate-list');
    const secEl = document.getElementById('mobile-candidate-section');
    const titleEl = document.getElementById('mobile-candidate-title');
...
function toggleUniv(uniqueKey) {
    ...
    const mobileTitleText = document.getElementById('mobile-candidate-title')?.textContent;
    ...
    if (mobileTitleText) {
        const mobileQuery = mobileTitleText.includes('검색') ? document.getElementById('mobile-univ-search').value.trim() : '';
        if (mobileQuery) {
            searchCandidatesMobile();
        }
    }
...
function renderActiveTags() {
    const active = getActiveTimetable();
    const containers = [
        document.getElementById('selected-univs'),
        document.getElementById('mobile-selected-univs')
    ].filter(Boolean);

    if (containers.length === 0) return;

    const html = active.univs.map(name => `
        <div class="tag">
            <span>${name}</span>
...
function renderGrid() {
    const active = getActiveTimetable();
    const containers = [
        document.getElementById('timetable-grid-wrapper'),
        document.getElementById('timetable-grid-wrapper-mobile')
    ].filter(Boolean);
    ...
    // 담은 대학 개수 배지 업데이트
    const countBadges = [
        document.getElementById('timetable-count-badge'),
        document.getElementById('timetable-count-badge-mobile')
    ].filter(Boolean);
    ...
    if (matchingRows.length === 0) {
        containers.forEach(c => {
            c.innerHTML = `<p style="text-align:center; padding:3rem 0; color:var(--text-muted);">담은 대학의 일정 데이터가 없습니다.</p>`;
        });
        return;
    }
...
function renderSummary() {
    ...
    const panels = [
        ...
        document.getElementById('summary-panel-mobile')
    ].filter(Boolean);
    
    const bodies = [
        document.getElementById('summary-body'),
        document.getElementById('mobile-summary-body')
    ].filter(Boolean);

    if (panels.length === 0 || bodies.length === 0) return;
    ...
    bodies.forEach(body => { body.innerHTML = tableHtml; });
        ...
        rows.forEach(row => {
            ...
            row.addEventListener('dragstart', (e) => {
                ...
                if (selectedRowKeyForMove) {
                    const prevSel = document.querySelectorAll('.selected-row-for-move');
                    prevSel.forEach(el => el.classList.remove('selected-row-for-move'));
            ...
            row.addEventListener('dragover', (e) => {
                ...
                if (targetRow && targetRow !== draggingRow && targetRow.parentNode === draggingRow.parentNode) {
...
function renderTabs() {
    const containers = [
        document.getElementById('timetable-tabs'),
        document.getElementById('timetable-tabs-mobile')
    ].filter(Boolean);

    if (containers.length === 0) return;

    const html = state.timetables.map(t => `
...
function updateDDay() {
    ...
    const dDayRightMobileEl = document.getElementById('dday-header-right-mobile');
    const dDayNumbersEl = document.getElementById('dday-numbers');
    const dDayNumbersMobileEl = document.getElementById('dday-numbers-mobile');
    
    if (diff <= 0) {
        if (dDayRightEl) dDayRightEl.textContent = '2026.11.19. / D-Day 경과';
        if (dDayRightMobileEl) dDayRightMobileEl.textContent = 'D-Day 경과';
        if (dDayNumbersEl) dDayNumbersEl.textContent = '00:00:00:00';
        if (dDayNumbersMobileEl) dDayNumbersMobileEl.textContent = '00:00:00:00';
    } else {
        ...
        if (dDayRightEl) dDayRightEl.textContent = `2026.11.19. / D-${dayDiff}`;
        if (dDayRightMobileEl) dDayRightMobileEl.textContent = `D-${dayDiff}`;
        
        const pad = (num) => String(num).padStart(2, '0');
        const formattedTime = `${counterDays}:${pad(diffHours)}:${pad(diffMinutes)}:${pad(diffSeconds)}`;
        if (dDayNumbersEl) dDayNumbersEl.textContent = formattedTime;
        if (dDayNumbersMobileEl) dDayNumbersMobileEl.textContent = formattedTime;
    }
}
...
function closeWelcomeModal(version) {
    const modal = document.getElementById('welcome-modal');
...
function selectVersion(version) {
    localStorage.setItem('nulsul_version', version);
    applyVersion(version);
}

function applyVersion(version) {
    const body = document.body;
    const versionBtnPC = document.getElementById('version-btn-pc');
    const versionBtnMobile = document.getElementById('version-btn-mobile');
    if (version === 'mobile') {
        ...
        if (versionBtnPC) versionBtnPC.style.display = 'flex';
        if (versionBtnMobile) versionBtnMobile.style.display = 'none';
    }
}
...
function renderInteractiveSchedule() {
    const containers = [
        document.getElementById('interactive-schedule-table'),
        document.getElementById('mobile-interactive-schedule-table')
    ].filter(Boolean);
    
    if (containers.length === 0 || state.univData.length === 0) return;
